...
It lets recipients view, download, and upload documents, yet prevents them from making changes (e.g., rename, move, collaborative editing). 
...
* The minimum version of pear/archive_tar was updated to be 1.4.7.
...
* Store quota overrides in the `oc_preferences` table https://github.com/owncloud/core/issues/34467[#34467]. In former versions, manual quota changes by the administrator were preserved against those values imported via user backends, such as LDAP. This fix restores the previous functionality.
