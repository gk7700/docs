...
* The `oc_share` table receives a new column (impact depends on the number of shares)
* The `oc_authtoken` table's login name column size has been increased (impact depends on the number of recently logged in users plus several app passwords)

=== PHP 5.6/7.0 Deprecation

Following the https://doc.owncloud.com/server/admin_manual/release_notes.html#php-5-6-deprecation[PHP 5.6/7.0 deprecation note with the release of ownCloud Server 10.0.8], ownCloud Server 10.2 *no longer supports PHP 5.6*. 
If you're still running version 5.6, you must upgrade to PHP 7 before upgrading to ownCloud Server 10.2. 
...
Also, some new app versions do not support older PHP versions anymore. These will not upgrade if your PHP version is not compatible.

To allow for additional upgrade time, version 10.2 still supports PHP 7.0 as some major Linux distributions continue to deliver support. Support for PHP 7.0 will be discontinued in an upcoming version of ownCloud 10, to enhance both security and performance. Please plan an upgrade soon.

Apart from this, ownCloud Server is also being prepared for PHP 7.3 support which will become available in the next, minor, version.
...
Especially, considering collaborative editing solutions, this addition provides the foundation for mode-based document sharing like "view-only", "comments-only" or "enforce change tracking", for example. In the future such advanced permissions will significantly improve the security as well as the usability for review processes, working on Office documents collaboratively or exchanging information securely.
...
With version 10.2, ownCloud Server officially supports storage encryption with master keys stored in hardware security modules (HSM). In contrast to the regular master key-based storage encryption which stores the keys on the storage, storage encryption with keys in an HSM allows administrators to completely prevent anyone who has access to the storage from accessing the data stored in ownCloud. 
...
* In addition to the option "_Allow username autocompletion in share dialog_" in the global 'Sharing' settings, users can now autonomously decide to opt-out from autocompletion to protect their privacy. When enabled, other users need to enter a user's full identifier to be able to share with them.
...
Repair steps are employed to clean up and resolve issues from former versions. Usually, they run during upgrades, but some scenarios make it necessary to rerun them. To save time when only specific steps need to be taken, administrators can now individually execute them using `occ maintenance:repair --list` and `occ maintenance:repair --single "<repair step>"`.
- Command for the first run wizard to reset for all users
In some cases, administrators customize the First Run Wizard in order to distribute information to users. Using `occ firstrunwizard:reset-all` you can reset the popup so that it will appear for each user upon their next login.
...
The `occ files:scan` command is used to scan resources on the storage and make them available in ownCloud. While previously it could only be used for all or single users and groups of users, you can now also execute it for groups where the group name contains a comma.
